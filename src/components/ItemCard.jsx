...
const ItemCard = ({ id,price, image, name, quantity , _id }) => {
  ...
  const removeFromCart = async(id)=>{
        const res = await axios.delete(`https://beer-shop-backend-3.onrender.com/api/remove-from-cart/${id}`)
        const data =  res.data
  ...
  const incrementQuantity = async (id)=>{
    const res = await axios.put(`https://beer-shop-backend-3.onrender.com/api/increment-quantity/${id}`)
  ...
  const decrementQuantity = async(id)=>{
    const res = await axios.put(`https://beer-shop-backend-3.onrender.com/api/decrement-quantity/${id}`)
    const data =  await res.data
