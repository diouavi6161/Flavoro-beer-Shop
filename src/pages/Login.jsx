...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axios.post("https://beer-shop-backend-3.onrender.com/api/login", { email, password });
      const data = res.data;
  ...
  return (
    <div className="flex justify-center items-center h-screen">
      ...
      >
        ...
        focus:border-green-300 text-gray-600"
          autoComplete="off"
        ...
        />
        <Link to="/forgot-password" className="text-xs text-gray-600 hover:underline cursor-pointer -mb-1">
          Forgot Password
        </Link>
        ...
        <p className="text-xs text-gray-600 flex gap-2 -mt-1">
          <span>Or</span>
          <Link to="/signup" className="hover:text-green-600">Create Your account</Link>
        </p>
      </form>
    </div>
  );
